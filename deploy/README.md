...
docker compose up -d
# 或指定版本
AKHUB_IMAGE=ghcr.io/hsmirage/akhub:1.1.11 docker compose up -d
```
...
| `/health/version` | `{"status":"ok","version":"1.1.11"}` | 确认部署的是哪个版本 |
...
less install.sh
sh install.sh --version v1.1.11 --service
```
...
```bash
# 1. 二进制
tar -xzf akhub-v1.1.11-linux-x86_64.tar.gz
sudo install -m755 akhub-v1.1.11-linux-x86_64/akhub /usr/local/bin/akhub
