...
流式完整 usage 结算、Responses 后台查询/取消及部分 HTTP 端点仍需后续补齐。详见项目根目录的
`2026-09-06_功能审查-Akhub-report.md`。

**仍待完成**（功能补齐、发布硬化与真实环境验收）：

| 能力 | 计划阶段 |
|---|---|
| 流式完整 usage 结算、Responses 无损流式输出保存、后台生命周期与缺失端点 | 阶段 2 / 4 |
| 大请求临时文件缓冲、流式统计与异常边界 | 阶段 2 / 6 |
| 官方 SDK 黑盒验收（§26.2） | 阶段 6 |
| Docker 构建、多架构镜像与 Linux 工件 | 阶段 6 |
| 真实上游接入，以及 Sub2API / New API 探针现场字段形状 | 阶段 6 |

常规自动化测试在本地已通过，但新增定向复现暴露的缺陷仍未修复，不能以测试数量替代完整验收。
真实环境项目仍需在具备 Docker 和测试账号的机器上执行。
